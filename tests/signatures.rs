...
async fn async_check_sig(
    ...
    let ee = CertificateDer::from(ee);
    let cert = webpki::EndEntityCert::try_from(&ee).unwrap();
    let alg = pki_types::AsyncWrapperImpl {algorithm: alg};
    cert.async_verify_signature(&alg, message, signature).await
}

#[tokio::test]
#[cfg(feature = "async-verify")]
async fn async_ecdsa_p256_key_and_ecdsa_p256_sha256_good_signature() {
    ...
    assert_eq!(async_check_sig(ee, ECDSA_P256_SHA256, message, signature).await, Ok(()));
}
