...
async fn async_check_cert(ee: &[u8], ca: &[u8]) -> Result<(), webpki::Error> {
    let ca = CertificateDer::from(ca);
    let anchors = &[anchor_from_trusted_cert(&ca).unwrap()];
    ...
    let ee = CertificateDer::from(ee);
    let cert = webpki::EndEntityCert::try_from(&ee).unwrap();
    cert.async_verify_for_usage(
        webpki::ALL_ASYNC_VERIFICATION_ALGS,
        anchors,
        &[],
        time,
        KeyUsage::client_auth(),
...
mod async_test {
    use super::*;
    #[tokio::test]
    async fn async_cert_with_no_eku_accepted_for_client_auth() {
        ...
        assert_eq!(async_check_cert(ee, ca).await, Ok(()));
    }

    #[tokio::test]
    async fn async_cert_with_clientauth_eku_accepted_for_client_auth() {
    ...
    #[tokio::test]
    async fn async_cert_with_both_ekus_accepted_for_client_auth() {
    ...
    #[tokio::test]
    async fn async_cert_with_serverauth_eku_rejected_for_client_auth() {
        ...
        assert_eq!(async_check_cert(ee, ca).await, Err(webpki::Error::RequiredEkuNotFound));
    }
}
